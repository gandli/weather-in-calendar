...
export async function getWeatherForecast(locationId: string, days: number = 7): Promise<WeatherEvent[]> {
  try {
    const url = `${QWEATHER_DAILY_API_BASE}/${days}d?location=${locationId}`;
    const response = await fetch(url, {
      headers: {
        'X-QW-Api-Key': QWEATHER_API_KEY,
      },
...
export async function getWeatherByCity(city: string): Promise<WeatherEvent[]> {
  ...
  return getWeatherForecast(locationId);
}
