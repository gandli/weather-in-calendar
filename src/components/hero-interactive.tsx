...
export function HeroInteractive({ initialWeather, defaultCity }: HeroInteractiveProps) {
    ...
    useEffect(() => {
        // If we have initial data and the user hasn't typed anything yet,
        // we might rely on the initial data.
        // However, to ensure consistency if the user types then clears, or just to keep the logic simple:
        const cityName = debouncedCity || defaultCity;

        // Optimization: If the city is the default city and we already have data (from initial props),
        // we could potentially skip fetching. But the initial props might be stale if the user sits on the page.
        // For simplicity and correctness, we will fetch, but the initial render is already covered by SSR data.
        // To avoid an immediate double-fetch on mount (SSR data vs Client fetch), we can check:
        if (!debouncedCity && previewWeather === initialWeather) {
             return;
        }
        ...
        fetchPreview();
    }, [debouncedCity, defaultCity, locale, initialWeather, previewWeather]);
