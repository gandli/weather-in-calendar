...
export function Hero() {
    ...
    const handleGenerate = () => {
        if (!city) return;
        setIsLoading(true);
        // Direct navigation to webcal protocol
        const url = `webcal://localhost:3000/${locale}/api/ics?city=${encodeURIComponent(city)}`;
        window.location.href = url;

        // Reset loading state after a short delay to allow the browser to hand off
